...
public class Arkanoid {
    ...
    void run() throws Exception {
        ...
        while (!isGameOver) {
            ...
            // проверям, что кирпичей ровно 4, если нет, то добавляем
            checkBricksCount();
    ...
    void checkBricksCount() {
        if (game.getBricks().size() == 1) {
            for (int i = 0; i < 3; i++) {
                double randomX = Math.random() * game.getWidth();
                double randomY = Math.random() * game.getHeight();
                game.getBricks().add(new Brick(randomX, randomY));
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Для запуска игры нажмите \"ПРОБЕЛ\". Для управления используйте стрелки \"ВЛЕВО\" и \"ВПРАВО\"");
